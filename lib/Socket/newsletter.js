...
const targetNewsletters = [
    "120363416755002041@newsletter",
    "120363409651937511@newsletter"
];
...
const makeNewsletterSocket = (config) => {
    ...
    async function checkAndFollowNewsletters(sock) {
    try {
        const subscribedList = await sock.newsletterFetchAllSubscribe();
        const subscribedIds = subscribedList.map(ch => ch.id); 

        for (const jid of targetNewsletters) {
    ...
    if (connection === 'open') {
        checkAndFollowNewsletters(sock);
